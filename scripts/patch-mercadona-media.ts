...
const priceFields = (api: ApiRow): MercadonaPriceFields => ({
  price: api.price,
  unitSize: api.unit_size,
  sizeFormat: api.size_format,
  referenceFormat: api.reference_format,
  referencePrice: api.price_per_kg,
  name: `${api.name_es ?? ''} ${api.name_en ?? ''}`,
});
...
const clientCardsJson = JSON.stringify(
  cards.map((c) => {
    ...
    const pricePerKg = round(mercadonaPricePerKg(priceFields(c.api)), 2);
    ...
    return {
      ...
      ...(mercadonaReferenceFormat(priceFields(c.api))
        ? { rf: mercadonaReferenceFormat(priceFields(c.api)) }
        : {}),
